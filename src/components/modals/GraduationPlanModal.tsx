...
import { faTimes, faGraduationCap, faCalendarAlt, faGripVertical, faExclamationTriangle, faBan, faTrash, faPlus, faCoins, faBook, faCodeBranch } from '@fortawesome/free-solid-svg-icons';
...
export default function GraduationPlanModal({ 
      ...
      for (let semesterIndex = 0; semesterIndex < updatedPlan.length; semesterIndex++) {
        ...
        if (subjectsToMove.length > 0) {
          ...
          subjectsToMove.forEach(subject => {
            ...
            semester.credits = semester.subjects.reduce((sum, s) => sum + getUcCredits(s), 0);
            ...
            for (let futureIndex = semesterIndex + 1; futureIndex < updatedPlan.length; futureIndex++) {
              ...
              if (canPlace) {
                ...
                updatedPlan[futureIndex].credits += getUcCredits(subject);
                placed = true;
                break;
              }
            }
  ...
  const recalculateTotals = () => {
    ...
    const approvedCredits = allSubjects
      ...
      .reduce((sum, subject) => sum + getUcCredits(subject), 0);
        ...
        >
          {miniAlert.visible && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 20 }}
              className="fixed z-70 left-1/2 bottom-8 md:bottom-12 -translate-x-1/2 bg-white/95 dark:bg-gray-800/95 border border-red-200 dark:border-red-700 shadow-xl rounded-2xl px-6 py-4 flex items-center gap-3"
              style={{ minWidth: 260, maxWidth: 340 }}
            >
              <FontAwesomeIcon icon={faExclamationTriangle} className="text-red-400 text-lg" />
              <span className="text-sm text-red-800 dark:text-red-300 font-semibold">{miniAlert.message}</span>
            </motion.div>
          )}
          <motion.div
            ...
            className={`bg-white/95 dark:bg-gray-800/95 backdrop-blur-lg rounded-3xl shadow-2xl border border-white/30 dark:border-gray-700 max-w-6xl w-full max-h-[98vh] md:max-h-[80vh] overflow-hidden flex flex-col ${darkMode ? 'dark' : ''}`}
          >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-white/80">
                {totalSemesters} semestres • {totalCredits} créditos UC • {yearsRemaining % 1 === 0 ? yearsRemaining : yearsRemaining.toFixed(1)} años restantes
              </p>
            </div>
          </div>
          <button
        ...
        {(draggedSubject || (isDraggingTouch && draggedSubject)) && (
          ...
          {localPlan.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h4 className="text-2xl font-bold text-gray-800 dark:text-white mb-2">¡Felicitaciones!</h4>
              <p className="text-gray-600 dark:text-gray-400">Ya has completado todas las asignaturas de la carrera.</p>
            </div>
          ) : (
            <>
              <div className="mb-4 p-0.5 md:p-1 rounded-2xl shadow border border-blue-100 dark:border-blue-700 bg-gradient-to-br from-blue-50/70 via-white/90 to-green-50/70 dark:from-blue-900/30 dark:via-gray-800/90 dark:to-green-900/30">
                <div className="p-3 md:p-4 flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900 flex items-center justify-center shadow">
                    <FontAwesomeIcon icon={faGripVertical} className="text-blue-500 dark:text-blue-400 text-base" />
                  </div>
                  <div className="flex-1">
                    <h5 className="font-bold text-base md:text-lg text-blue-700 dark:text-blue-400 mb-0.5 flex items-center gap-2">
                      Reorganiza tu plan
                    </h5>
                    <div className="text-xs text-blue-900/80 dark:text-blue-300/80 font-medium space-y-0.5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {localPlan.slice(0, visibleSemesters).map((semesterPlan, index) => {
                  ...
                  return (
                    <div 
                      ...
                      className={`bg-white/50 dark:bg-gray-700/50 backdrop-blur-sm rounded-2xl p-4 border transition-all duration-300 transform ${
                        isAnimating && index === visibleSemesters - 1 
                          ? 'scale-105 ring-2 ring-blue-400 shadow-lg border-blue-200' 
                          : wouldExceedWithDrag
                          ? 'scale-105 ring-2 ring-red-400 shadow-lg border-red-200 bg-red-50/50 dark:bg-red-900/20'
                          : dragOverSemester === semesterPlan.semester
                          ? 'scale-105 ring-2 ring-green-400 shadow-lg border-green-200 bg-green-50/50 dark:bg-green-900/20'
                          : isOverloaded
                          ? 'border-orange-300 dark:border-orange-500 bg-orange-50/30 dark:bg-orange-900/20'
                          : 'scale-100 border-gray-200 dark:border-gray-600'
                      }`}
                    ...
                    >
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <div className="flex-1">
                          ...
                          <p className={`text-xs ${
                            isOverloaded ? 'text-orange-600 dark:text-orange-400 font-medium' : 'text-gray-600 dark:text-gray-400'
                          }`}>
                        ...
                        {semesterPlan.subjects.length === 0 && (
                          <button
                            className="no-print ml-2 p-1 rounded-full bg-red-100 dark:bg-red-900/30 hover:bg-red-200 dark:hover:bg-red-900/50 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-700 transition-colors"
                      ...
                      {wouldExceedWithDrag && (
                        <div className="border-2 border-dashed border-red-400 dark:border-red-500 rounded-lg p-2 text-center text-red-600 dark:text-red-400 text-xs mb-2 bg-red-50/30 dark:bg-red-900/20">
                      ...
                      {!wouldExceedWithDrag && ((semesterPlan.subjects.length === 0 && dragOverSemester === semesterPlan.semester) || 
                        ...
                        <div className="border-2 border-dashed border-green-400 dark:border-green-500 rounded-lg p-2 text-center text-green-600 dark:text-green-400 text-xs mb-2 bg-green-50/30 dark:bg-green-900/20">
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className="text-white/80 text-xs">
                                      {getUcCredits(subject)} créditos UC
                                    </div>
                                  </div>
                                </div>
                              </div>
                ...
                <button
                  className="no-print h-full min-h-[180px] flex flex-col items-center justify-center border-2 border-dashed border-blue-300 dark:border-blue-600 bg-white/60 dark:bg-gray-700/60 rounded-2xl shadow-sm hover:bg-blue-50 dark:hover:bg-blue-900/30 hover:border-blue-400 dark:hover:border-blue-500 transition-all focus:outline-none focus:ring-2 focus:ring-blue-300 dark:focus:ring-blue-600"
                  onClick={() => {
              ...
              <div className="mt-6 p-1 md:p-2 rounded-3xl shadow-xl border border-blue-100 dark:border-gray-700 bg-white dark:bg-gray-800">
                <div className="p-5 md:p-8">
                  <h5 className="font-extrabold text-lg md:text-2xl text-blue-700 dark:text-blue-400 mb-4 flex items-center gap-2">
                    <FontAwesomeIcon icon={faGraduationCap} className="text-blue-500 text-xl md:text-2xl" />
                    Resumen del Plan
                  </h5>
                  {localPlan.some(s => s.credits > 60) && (
                    <div className="mb-4 flex rounded-xl shadow border border-orange-300 dark:border-orange-700 bg-orange-50 dark:bg-orange-900/20 relative overflow-hidden">
                      ...
                      <div className="flex-1 p-4">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-base font-extrabold text-orange-800 dark:text-orange-400">Sobrecarga Académica</span>
                        </div>
                        <div className="text-xs text-orange-700 dark:text-orange-300 space-y-1 font-medium mb-2">
                          {localPlan.filter(s => s.credits > 55).map(semester => {
                            const overloadCredits = semester.credits - 60;
                            return (
                              <div key={semester.semester}>
                                <span className="inline-block font-bold text-orange-900 dark:text-orange-200">{semester.semester}</span>: {semester.credits} créditos UC (sobrecarga de {overloadCredits} créditos UC)
                              </div>
                            );
                          })}
                        </div>
                        <div className="text-xs text-orange-600 dark:text-orange-400 mt-2 pt-2 border-t border-orange-200 dark:border-orange-700">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-4">
                    <div className="text-center flex flex-col items-center">
                      ...
                      <div className="text-2xl font-extrabold text-blue-700 dark:text-blue-400">{totalSemesters}</div>
                      <div className="text-blue-900/80 dark:text-blue-300/80 font-medium">Semestres</div>
                    </div>

                    <div className="text-center flex flex-col items-center">
                      <div className="w-12 h-12 rounded-full bg-green-100 dark:bg-green-900/50 flex items-center justify-center mb-1">
                        <FontAwesomeIcon icon={faCoins} className="text-green-500 text-xl" />
                      </div>
                      <div className="text-2xl font-extrabold text-green-700 dark:text-green-400">{approvedCredits}/{approvedCredits + totalCredits}</div>
                      <div className="text-green-900/80 dark:text-green-300/80 font-medium">Créditos UC</div>
                    </div>

                    <div className="text-center flex flex-col items-center">
                      <div className="w-12 h-12 rounded-full bg-purple-100 dark:bg-purple-900/50 flex items-center justify-center mb-1">
                        <FontAwesomeIcon icon={faGraduationCap} className="text-purple-500 text-xl" />
                      </div>
                      <div className="text-2xl font-extrabold text-purple-700 dark:text-purple-400">{yearsRemaining % 1 === 0 ? yearsRemaining : yearsRemaining.toFixed(1)}</div>
                      <div className="text-purple-900/80 dark:text-purple-300/80 font-medium">Años Restantes</div>
                    </div>
                    <div className="text-center flex flex-col items-center">
                      <div className="w-12 h-12 rounded-full bg-orange-100 dark:bg-orange-900/50 flex items-center justify-center mb-1">
                        <FontAwesomeIcon icon={faBook} className="text-orange-500 text-xl" />
                      </div>
                      <div className="text-2xl font-extrabold text-orange-700 dark:text-orange-400">{totalSubjects}</div>
                      <div className="text-orange-900/80 dark:text-orange-300/80 font-medium">Asignaturas</div>
                    </div>
                  </div>

                  <div className="mt-2 text-sm text-blue-900/80 dark:text-blue-300/80 text-center font-medium">
                  ...
                  <div className="flex justify-center w-full mt-6">
                    <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-2xl shadow-sm p-4 text-xs text-yellow-900 dark:text-yellow-300 text-center max-w-xl w-full flex items-center gap-2">
          ...
          {isAnimating && localPlan.length > 0 && (
            <div className="mt-6 text-center">
              <div className="inline-flex items-center gap-2 bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-300 px-4 py-2 rounded-full">
